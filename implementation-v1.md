...
| L2 | `sierra_runtime_topology` | controller/worker charts, backend contract, readiness, permissions (**and opt-in L2 authoritative computation/emission hooks**) | `/execution`, `/execution/backend` |
| L3 | `studies_features` | studies binding, modes, inputs/outputs mapping, bind policies (**and opt-in L3 evaluation configuration**) | `/studies` (+ validation policies for study input mutation) |
...
## Augmentation (Additive, Non-Breaking): First-Class Indicator Chaining + EV Evaluation (NEW)

This section is an **additive augmentation** to the v2.6.2 plan bundle that introduces:

- **Layer 2 indicator chaining as a first-class, typed DAG** (`execution.layer2.indicator_dag`)
- **Layer 2 event/opportunity emission** (`execution.layer2.event_emitter`)
- **Layer 3 outcomes + bucketing + expected-value evaluation** (additional optional blocks under `outputs.layer3`)

### Compatibility Rules (Normative)

- If `execution.layer2` is absent: runtime MUST behave exactly as before.
- If `outputs.dataset.source` is absent: default is `"bars"`.
- If `outputs.dataset.source == "layer2_event_emitter"`: dataset rows come from the Layer 2 event emitter.
- If Layer 3 new blocks are absent: existing `outputs.layer3.mode` behavior (e.g., `rr_menu`) MUST remain unchanged.

### Layer 2 Augmented Responsibilities (Normative)

When `execution.layer2.enabled == true`, Layer 2 additionally owns:

1) **Indicator DAG Evaluation**
   - DAG MUST be acyclic.
   - Evaluation order MUST be deterministic (stable topo-sort).
   - Node dependencies MUST be explicit.
   - History-bearing nodes MUST declare warmup/lookback semantics.

2) **Opportunity/Event Emission**
   - `emit_mode` supports:
     - `on_true_edge` (false→true transitions)
     - `on_true` (every bar true)
   - Cooldown/dedupe semantics MUST be deterministic.
   - Event dataset is considered authoritative for downstream evaluation.

Layer 2 MUST NOT compute outcomes (MFE/MFA) or aggregate statistics.

### Layer 3 Augmented Responsibilities (Normative)

When configured (opt-in), Layer 3 additionally owns:

1) **Outcome Computation**
   - Outcomes MUST start at `t+1` (no same-bar overlap).
   - MFE/MFA MUST be computed over the declared horizon.
   - Cost models (e.g., fixed bps) are applied here.

2) **Bucketing / Conditioning**
   - Quantile bucketing MUST declare distribution scope.
   - Quantile method MUST be deterministic.

3) **Expected Value Metrics**
   - Bucket metrics MUST include: `n_trades` and `EV`.
   - Percentile outputs for MFE/MFA MUST be supported.

Layer 3 MUST NOT redefine or filter the Layer 2 opportunity set.

### New Error Codes (Normative)

If the runtime implements the augmented behavior, it MUST introduce specific error codes (or equivalent) rather than generic failures:
...
### Required Fixtures (Normative)

The augmentation MUST be locked with fixtures/tests:

**Layer 2 fixtures**
- valid DAG + event emission
- cycle DAG → fails with `E_L2_DAG_CYCLE`
- missing dep → fails with `E_L2_MISSING_NODE_DEP`
- unknown node kind → fails with `E_L2_UNKNOWN_NODE_KIND`
- emit_mode semantics (`on_true_edge` vs `on_true`) deterministically validated

**Layer 3 fixtures**
- outcomes computed with strict `t+1` rule
- fixture that would leak same-bar → fails with `E_L3_OUTCOME_LEAKAGE_SAME_BAR`
- quantile bucketing determinism (same inputs → identical bucket boundaries)
